...
def test_whole_by_golden(golden, caplog):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        output_stream = "examples/output.txt"
        ...
        with contextlib.redirect_stdout(io.StringIO()) as stdout:
            ...
            machine.main([target, input_file, output_stream])

        # Выходные данные также считываем в переменные.
        with open(target, encoding="utf-8") as file:
            code = file.read()

        with open(output_stream, encoding="utf-8") as out_file:
            output = out_file.read()

        with open("examples/output.txt", "w") as clean_out:
            clean_out.write("")
